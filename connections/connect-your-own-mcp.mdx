...
Use **Custom MCP** when a service exposes a public MCP server. Use **Custom API** when it gives you an API key and an HTTP API instead.

Add a custom connection only when a specific job needs it, then start with the narrowest tool permissions that work. Both options live in **Connected Apps**.
