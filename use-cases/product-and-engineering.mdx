...
14 open issues, 6 rolled over from last sprint, 3 are blocked. The onboarding flow you shipped last sprint has 23% drop-off at step 3. Runner recommends fixing that first, then 2 high-priority bugs. Runner also updates the sprint in Linear with suggested assignments.

### Trace a bug to the PR that caused it

Something is broken. Runner checks your analytics for the drop, finds the deploy that lines up with the timing, pulls the diff, identifies the change, and files a P1 bug in Linear for the PR author.

> *"We're getting reports of a checkout bug, what's going on?"*

Conversion dropped 18% starting yesterday at 3pm. Deploy at 2:45pm, PR #847 touched the payment flow. Null check was removed on line 142. Runner files the bug and assigns it to the PR author.

### Measure feature impact

Runner pulls usage events from Mixpanel, checks session recordings, identifies gaps, and creates the next action item. Not a dashboard you have to interpret.

> *"How is the new search feature performing?"*

2,400 searches per day since launch. 34% return zero results. Top query: integrations. Runner creates a Linear issue to add the integrations page to the search index. Fixing it is a 1-point ticket.
...
| **10:00 AM** | Bug triage: traces a reported issue to the PR that caused it and files a P1 |
| **11:00 AM** | Feature impact review: pulls usage data, identifies gaps, and creates tickets |
...
- **Post-deploy monitoring** after every release. Error rates, funnels, and a summary for #engineering.
